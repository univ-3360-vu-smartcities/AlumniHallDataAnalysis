...
DATASOURCE = ['BdX']
DATATYPE = ['.csv', '.xlsx', '.pkl', '.txt']


from datautils.decorators import timer
...
@register
def readtxt(read_path):

	df = pd.read_table(read_path, sep = ',', header = 0)
...
def df_minmax_scaling(df: pd.DataFrame, feature_range=(0,1)):
	"""
	Scale the input dataframe to the feature_range
	Arguments:
		df {pd.DataFrame} -- DataFrame to scale
	Keyword Arguments:
		feature_range {tuple} -- min and max of the range for scaling. (default = (0,1))
	Returns:
		Scaled Dataframe, Scaler object in case it is needed for inverse transform to original scale
	"""
	...
	# Scale the data in the required range
	df = pd.DataFrame(x_sc.transform(x), columns=df.columns, index = df.index)

	return df,x_sc

def df_standardizing(df: pd.DataFrame):
	"""
	Normalize the input dataframe
	Arguments:
		df {pd.DataFrame} -- DataFrame to standardize
	Returns:
		Standardized Dataframe, Scaler object in case it is needed for inverse transform to original scale
	"""
